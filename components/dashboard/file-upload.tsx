...
export function FileUpload({ studyBuddyId }: { studyBuddyId: string }) {
  ...
  const { documents, uploadDocument } = useDocuments(studyBuddyId);
  const isLimitReached = (documents as any[]).length >= 1;
    ...
    <div className="relative group">
      ...
      <div 
        ...
        className={`
          ...
          ${isUploading || isLimitReached ? "bg-slate-50 border-slate-200 cursor-not-allowed" : "bg-white border-slate-200 hover:border-primary hover:bg-primary/5 shadow-sm hover:shadow-md group-hover:scale-[1.01] cursor-pointer"}
        `}
      >
        <div className={`p-4 rounded-2xl mb-4 transition-colors 
          ${isUploading || isLimitReached ? "bg-slate-100 text-slate-400" : "bg-primary/10 text-primary group-hover:bg-primary group-hover:text-white"}`}>
           {isUploading ? <Loader2 className="w-8 h-8 animate-spin" /> : <FileUp className="w-8 h-8" />}
        </div>
        
        <h3 className="text-xl font-bold text-slate-900 mb-1">
          {isUploading ? "Processing Document..." : isLimitReached ? "Limit Reached" : "Upload Knowledge Source"}
        </h3>
        <p className="text-slate-500 text-sm font-medium text-center max-w-xs">
          {isUploading ? "We're preparing your buddy with new info." : isLimitReached ? "This Buddy already has a knowledge source. Delete it to upload a new one." : "Click to browse or drag and drop PDF, DOCX, or PPTX files (Max 10MB)."}
        </p>
      </div>
