...
>(
  (
    ...
    // We use useControlledState so that the sidebar can be either controlled or uncontrolled.
...
>(
  (
    ...
    if (isMobile) {
      return (
        // Mobile Sidebar Implementation (Simplified Sheet-like)
        <div 
          className={cn(
        ...
        >
           <div
            className={cn(
              "fixed inset-y-0 left-0 z-50 w-[80%] max-w-[--sidebar-width] bg-sidebar p-0 shadow-lg transition-transform duration-300 ease-in-out sm:max-w-sm",
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "peer-data-[variant=inset]:min-h-[calc(100svh-theme(spacing.4))] md:peer-data-[variant=inset]:m-2 md:peer-data-[active=true]:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow",
        className
      )}
      {...props}
    />
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    ...
    >
      ...
      <div className="h-4 flex-1 rounded-md bg-sidebar-accent/50" />
    </div>
  )
})
